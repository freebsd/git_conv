...
rewrite_tag()
{
    ...
    c_committer=${c_auth%<*}
    c_email=${c_auth#*<}
    t_date=`git cat-file commit "$target^{commit}" | sed -n '/^committer/s/^committer //p' | egrep -o '[0-9]* [+0-9]*$'`
...
git for-each-ref --format='%(refname:short)' refs/tags |
    ...
    while read rev; do
        tag_tree=`git show -s --format=%t "$rev^{}"`
        parent_tree=`git show -s --format=%t "$rev~1" 2>/dev/null`
